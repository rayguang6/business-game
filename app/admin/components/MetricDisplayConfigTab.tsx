...
export function MetricDisplayConfigTab({
  ...
  const sortedMetrics = useMemo(() => {
    if (!sortBy) return allMetrics;

    return [...allMetrics].sort((a, b) => {
      const aPriority = getDisplayValue(a.id, 'priority') as number | null;
      const bPriority = getDisplayValue(b.id, 'priority') as number | null;

      // Handle null values (put them at the end)
      if (aPriority === null && bPriority === null) return 0;
      if (aPriority === null) return sortDirection === 'asc' ? 1 : -1;
      if (bPriority === null) return sortDirection === 'asc' ? -1 : 1;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-lg p-8">
        <div className="mb-6">
          <h2 className="text-3xl font-semibold mb-3">Metric Display Configuration</h2>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-800 border-b border-slate-700">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Label</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Description</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Show on HUD</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Show in Details</th>
                  <th
                  ...
                  <th className="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Unit</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Icon Path</th>
                  ...
                  return (
                    <tr
                      ...
                      className={`hover:bg-slate-800/50 transition ${
                        hasChange ? 'bg-yellow-900/10 border-l-2 border-yellow-500' : ''
                      }`}
                    >
                      {/* Label */}
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <input
                            ...
                            className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm text-slate-200 focus:outline-none focus:ring-1 focus:ring-blue-500"
                          ...
                          <span className="text-xs text-slate-500 mt-1 font-mono">{metric.id}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <textarea
                            ...
                            className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm text-slate-200 focus:outline-none focus:ring-1 focus:ring-blue-500 min-w-[200px]"
                      ...
                      <td className="px-6 py-4">
                        <label className="flex items-center gap-2 cursor-pointer">
                          ...
                          />
                          {isGlobal && referenceIndustryId && refShowOnHUD !== null && refShowOnHUD !== showOnHUD && (
                            <span className="text-xs text-slate-500 italic">
                              {referenceIndustryId}: {refShowOnHUD ? 'Yes' : 'No'}
                            </span>
                          )}
                        </label>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            ...
                            className="rounded bg-slate-800 border-slate-700 text-blue-600 focus:ring-blue-500"
                          />
                          {isGlobal && referenceIndustryId && refShowInDetails !== null && refShowInDetails !== showInDetails && (
                            <span className="text-xs text-slate-500 italic">
                              {referenceIndustryId}: {refShowInDetails ? 'Yes' : 'No'}
                            </span>
                          )}
                        </label>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          ...
                          />
                          {isGlobal && referenceIndustryId && refUnit !== null && refUnit !== unit && (
                            <span className="text-xs text-slate-500 mt-1 italic">
                              {referenceIndustryId}: {refUnit}
                            </span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <input
                            ...
                            className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm text-slate-200 min-w-[150px] focus:outline-none focus:ring-1 focus:ring-blue-500"
      ...
      {hasChanges() && (
        <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 z-50">
          <div className="bg-slate-900/95 backdrop-blur-sm border border-slate-700 rounded-xl px-8 py-4 shadow-2xl">
            <button
              onClick={saveAll}
